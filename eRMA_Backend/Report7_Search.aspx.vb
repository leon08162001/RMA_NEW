...
#End Region
    ...
    Private Sub setControls()
        ...
        Me.UI_lblTittle.Text = _oLanguage.getText("Report", "007", ctlLanguage.eumType.Tag)
        ...
        '身分:1.客戶, 2.公司(維修中心)
        Me.UI_trCompanyName.Visible = False
        If Session("_Identity").ToString().Trim() = "2" Then
            Me.UI_trCompanyName.Visible = True
        End If
    ...
    Private Sub QueryData(ByVal iPageIndex As Integer)
        ...
        Dim dtReport As New ReportDTO.vwRpt_CustomersRequestDataTable
        ...
        dtReport = ctlReport.QueryCustomersRequest(Session("_CustomerID").ToString().Trim(), CompanyName, Warranty, Repair, fdate, edate)
    ...
    Private Sub ArrangementData(ByVal dvReport As DataView)
        ...
        Dim iSubRequestQTY As Integer = 0
        Dim iSubClosedQTY As Integer = 0
        Dim iSubCanceledQTY As Integer = 0

        Me.ViewState("_TotalRequestQTY") = 0
        Me.ViewState("_TotalClosedQTY") = 0
        Me.ViewState("_TotalCanceledQTY") = 0

        Dim dtReport_Tmp As DataTable = dvReport.Table.Copy
        Dim dvReport_Tmp As DataView = dtReport_Tmp.DefaultView

        dvReport.Sort = "RMA_CUNO asc, RequestYM asc"
        For i = 0 To dvReport.Count - 1
            Dim RMA_CUNO As String = dvReport(i)("RMA_CUNO").ToString().Trim()
            ...
            If i = 0 Then
                ...
                dvReport(i)("isShow") = 1
            Else
                If sGroupBY.ToString().ToLower() <> RMA_CUNO.ToLower() Then
                    dvReport(i)("isShow") = 1
                    sGroupBY = RMA_CUNO
                End If
            End If
        ...
        sGroupBY = Nothing
        dvReport_Tmp.Sort = "RMA_CUNO asc, RequestYM asc"
        For i = 0 To dvReport_Tmp.Count - 1
            Dim RMA_CUNO As String = dvReport_Tmp(i)("RMA_CUNO").ToString().Trim()
            Dim iRequestQTY As Integer = Convert.ToInt16(dvReport_Tmp(i)("RequestQTY").ToString().Trim())
            Dim iClosedQTY As Integer = Convert.ToInt16(dvReport_Tmp(i)("ClosedQTY").ToString().Trim())
            Dim iCanceledQTY As Integer = Convert.ToInt16(dvReport_Tmp(i)("CanceledQTY").ToString().Trim())

            If i = 0 Then
                sGroupBY = RMA_CUNO
                iSubRequestQTY = iRequestQTY
                iSubClosedQTY = iClosedQTY
                iSubCanceledQTY = iCanceledQTY

            Else
                If sGroupBY.ToString().ToLower() <> RMA_CUNO.ToLower() Then
                    Dim dr As DataRow = dvReport.Table.NewRow
                    dr("RMA_CUNO") = sGroupBY.ToString.Trim()
                    dr("CU_NAME") = _oLanguage.getText("Report", "141", ctlLanguage.eumType.Tag)
                    dr("RequestYM") = "zzz"

                    Me.ViewState("_TotalRequestQTY") = Convert.ToInt16(Me.ViewState("_TotalRequestQTY")) + Convert.ToInt16(iSubRequestQTY)
                    Me.ViewState("_TotalClosedQTY") = Convert.ToInt16(Me.ViewState("_TotalClosedQTY")) + Convert.ToInt16(iSubClosedQTY)
                    Me.ViewState("_TotalCanceledQTY") = Convert.ToInt16(Me.ViewState("_TotalCanceledQTY")) + Convert.ToInt16(iSubCanceledQTY)

                    dr("RequestQTY") = Convert.ToInt16(iSubRequestQTY)
                    dr("ClosedQTY") = Convert.ToInt16(iSubClosedQTY)
                    dr("CanceledQTY") = Convert.ToInt16(iSubCanceledQTY)
                ...
                Else
                    iSubRequestQTY = Convert.ToInt16(iSubRequestQTY) + iRequestQTY
                    iSubClosedQTY = Convert.ToInt16(iSubClosedQTY) + iClosedQTY
                    iSubCanceledQTY = Convert.ToInt16(iSubCanceledQTY) + iCanceledQTY
                End If
            End If
    ...
    Private Sub Request_DataBind(ByVal dvReport As DataView, ByVal iPageIndex As Integer)
        ...
        dvReport.Sort = "RMA_CUNO asc, isShow asc"
    ...
    Protected Sub UI_dvReport_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles UI_dvReport.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.Header Then
            ...
            e.Row.Cells(3).Text = _oLanguage.getText("Report", "145", ctlLanguage.eumType.Tag)
            e.Row.Cells(4).Text = _oLanguage.getText("Report", "146", ctlLanguage.eumType.Tag)
            e.Row.Cells(5).Text = _oLanguage.getText("Report", "147", ctlLanguage.eumType.Tag)
            e.Row.Cells(6).Text = _oLanguage.getText("Report", "148", ctlLanguage.eumType.Tag)
        End If

        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim UI_CU_NAME As Label = e.Row.FindControl("UI_CU_NAME")
            ...
            Select Case UI_isShow.Text.Trim()
                ...
                Case "3"
                    ...
                    UI_CU_NAME.Text = UI_CU_NAME.Text.Trim() & "&nbsp;&nbsp;"
            End Select
        End If
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            Dim UI_Footer_RequestQTY As Label = e.Row.FindControl("UI_Footer_RequestQTY")
            UI_Footer_RequestQTY.Text = Me.ViewState("_TotalRequestQTY").ToString().Trim()

            Dim UI_Footer_ClosedQTY As Label = e.Row.FindControl("UI_Footer_ClosedQTY")
            UI_Footer_ClosedQTY.Text = Me.ViewState("_TotalClosedQTY").ToString().Trim()

            Dim UI_Footer_CanceledQTY As Label = e.Row.FindControl("UI_Footer_CanceledQTY")
            UI_Footer_CanceledQTY.Text = Me.ViewState("_TotalCanceledQTY").ToString().Trim()
        End If
