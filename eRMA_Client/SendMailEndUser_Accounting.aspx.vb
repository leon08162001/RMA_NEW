...
Partial Class _SendMailEndUser_Accounting
    ...
    '回填RMA客戶同意報價單
    Public Function Up_RMAREPAIR_QUOTED(ByVal RMARQ_RMADID As String, ByVal RMARQ_ACCEPT As String) As Boolean
        Dim myConnection As New OracleConnection("DATA SOURCE=192.168.7.20:1521/topprod;USER ID=rma;PASSWORD=4321rma;PERSIST SECURITY INFO=True;")
        myConnection.Open()
        ...
        Try
            myCommand.CommandText = "  UPDATE RMAREPAIR_QUOTED SET RMARQ_ACCEPT =:RMARQ_ACCEPT WHERE RMARQ_RMADID =:RMARQ_RMADID  "

            myCommand.Parameters.AddWithValue("RMARQ_ACCEPT", RMARQ_ACCEPT)
            myCommand.Parameters.AddWithValue("RMARQ_RMADID", RMARQ_RMADID)
            ...
            If Me.IsPostBack = False Then
                ...
                Try
                    ...
                    For i = 0 To dvRMAREPAIR_QUOTED_Client.Count() - 1
                        ...
                        Dim RMARQ_ACCEPT As String = dvRMAREPAIR_QUOTED_Client(i)("RMARQ_ACCEPT").ToString().Trim()
                        Up_RMAREPAIR_QUOTED(RMARQ_RMADID, RMARQ_ACCEPT)

                    Next
                    '最後維修寄信,在修改一次RMA客戶同意報價單  結束
