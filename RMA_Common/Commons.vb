...
Public Class Commons
    ...
    Dim _isDebugSIT As String = ConfigurationManager.AppSettings("isDebugSIT")
...
#End Region
    ...
    '    '               Path.Combine(ConfigurationManager.AppSettings("Report_FilePath"), ReportToPDF))
    '    reportDoc.Export()
    'End Sub
    Public Sub ExportSetup_New(rptPath As String, exportPath As String, ds As DataSet)
        ...
        Try
            ...
            ' JSON Path（先用實體路徑驗證）
            'Dim jsonPath As String = "C:\Temp\temp_data.json"

            ' 保險序列化
            Dim dict As New Dictionary(Of String, Object)
            For Each tbl As DataTable In ds.Tables
                dict(tbl.TableName) = tbl
            Next

            Dim jsonPath As String = HttpContext.Current.Server.MapPath("~/Report/temp_data.json")
            Dim jsonData As String = JsonConvert.SerializeObject(dict, Formatting.Indented)
            File.WriteAllText(jsonPath, jsonData, Encoding.UTF8)
            Dim proc As New Process()
            proc.StartInfo.FileName = HttpContext.Current.Server.MapPath("~/bin/RMA_ReportExportPDFHost.exe")
            proc.StartInfo.Arguments = String.Format("""{0}"" ""{1}"" ""{2}""", rptPath, exportPath, jsonPath)
            ...
            LogHelper.WriteLog("報表匯出完成：" & exportPath)
        Catch ex As Exception
            LogHelper.WriteLog("報表匯出錯誤：" & ex.ToString())
        End Try
    End Sub
    ...
    Public Sub OpenPdf(page As Page, pdfFileName As String)
        ...
        Dim pdfUrl As String = If(HttpContext.Current.Request.IsLocal OrElse (_isDebug = True And _isDebugSIT = False), scheme & "://" & host & ":" & port, _WEBURL) & "/ashx/ExportPdfHandler.ashx?file=" & pdfFileName
    ...
    ''' <summary>
    ''' 安全跳出 JavaScript alert 視窗
    ''' </summary>
    ''' <param name="page">當前 Page 對象</param>
    ''' <param name="msg">要顯示的訊息</param>
    ''' alert共用避免特殊字元造成沒反應 by buck add 20251201
    Public Sub AlertMessage(page As Page, msg As String)
        If String.IsNullOrEmpty(msg) Then Exit Sub

        ' 處理特殊字元
        Dim safeMsg As String = msg
        safeMsg = safeMsg.Replace("\", "\\")        ' 反斜線
        safeMsg = safeMsg.Replace("'", "\'")        ' 單引號
        safeMsg = safeMsg.Replace("""", "\""")      ' 雙引號
        safeMsg = safeMsg.Replace(vbCrLf, "\n")     ' Windows 換行
        safeMsg = safeMsg.Replace(vbLf, "\n")       ' Linux 換行
        safeMsg = safeMsg.Replace(vbCr, "\n")       ' Mac 換行

        ' 註冊 ScriptManager
        ScriptManager.RegisterStartupScript(page, page.GetType(), Guid.NewGuid().ToString(), "alert('" & safeMsg & "');", True)
    End Sub
End Class
