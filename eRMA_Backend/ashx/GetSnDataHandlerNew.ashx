...
Public Class GetSnDataHandlerNew : Implements IHttpHandler
    ...
    Private Function GetSnData(ByVal DistNumber As String) As Object
        Dim ConnectionString As String = "Data Source = 10.1.1.154; Initial Catalog =CipherLabs_PRD; User ID = data_query; Password = DaQu#0103;"
        ...
        Try
            ...
            Dim sql As String = "select ItemCode, Dscription,convert(varchar, GrnStart, 111) GrnStart, convert(varchar,GrntExp_Cal, 111)GrntExp_Cal  from ( " &
                ...
                "FROM [CipherLabs_PRD].[dbo].[ODLN] a " &
                "inner join [CipherLabs_PRD].[dbo].[DLN1] b ON a.DocEntry = b.DocEntry   " &
                "inner join [CipherLabs_PRD].[dbo].[OITL] c on b.DocEntry = c.ApplyEntry and b.LineNum = c.ApplyLine and c.ApplyType = 15 " &
                "inner join [CipherLabs_PRD].[dbo].[ITL1] d on c.LogEntry = d.LogEntry " &
                "inner join [CipherLabs_PRD].[dbo].[OSRN] e on d.ItemCode = e.ItemCode and d.MdAbsEntry =e.AbsEntry " &
                "where  " &
