...
Partial Class ProductInformation_05_002
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.IsPostBack = False Then
            ...
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "ShowInfoPage", "window.location = 'Client_Status_List.aspx';", True)
        End If
    End Sub
    ...
    Private Sub Print(ByVal dtReport As RmaDTO.RequestReportDataTable, ByVal EndUser As Boolean, ByVal PartsRequest As Boolean, ByVal sLanguageID As String)
        ...
        'add by Hugh 20250314, 加入check_bool的判斷
        ...
        If (Session("_RepairID").ToString() = "JP_BYTE" OrElse Session("_RepairID").ToString() = "AU" OrElse Session("_RepairID").ToString() = "NZ_PB_TECH" OrElse Session("_RepairID").ToString() = "AU_LAPTOP_KINGS") Then
            check_bool = True
        End If
        'add by Hugh 20250314, 加入check_bool的判斷 end
        'CrystalReport共用版本 by buck add 20251201 begin
        If (EndUser) Then
            If sLanguageID = "003" Then
                If (check_bool) Then
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_EndUser_jp_Notice.rpt"))
            ...
            Else
                If (check_bool) Then
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_EndUser_Notice.rpt"))
        ...
        Else
            If sLanguageID = "003" Then
                If (check_bool) Then
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_02_jp_Notice.rpt"))
            ...
            Else
                If (check_bool) Then
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_02_Notice.rpt"))
        ...
        'Note by Hugh, 看起來是要列印part request的pdf --> 先改成有Load, 之前是只有指定pdf路徑
        If (PartsRequest) Then
            If sLanguageID = "003" Then
                If (check_bool) Then
                    '原本 Server_MapPath_rpt = Server.MapPath("Report\rptRequest_Parts_jp_Notice.rpt")
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_Parts_jp_Notice.rpt"))
            ...
            Else
                If (check_bool) Then
                    'ReportDoc.Load(Server.MapPath("Report\rptRequest_Parts_Notice.rpt"))
            ...
            End If
        End If
        'CrystalReport共用版本 by buck add 20251201 end
